...
//	private Animator cloudanim;
	...
	void Update () {
        ...
        {
            if(lookingRight)
                Hammer.GetComponent<HammerController>().Direction = HammerController.Directions.Right;
            else
                Hammer.GetComponent<HammerController>().Direction = HammerController.Directions.Left;

            Instantiate(Hammer, new Vector3(transform.position.x, transform.position.y, transform.position.z), transform.rotation);
        }
	}
